...
const { data: projectsResponse } = await useApi<any>("/projects?populate=*");

// Хелпер для скриншотов
const getImg = (project: any) => {
  const path = project.attributes?.screenshot?.data?.attributes?.url || project.screenshot?.url;
  return path ? (path.startsWith('http') ? path : `${strapiHost}${path}`) : null;
};
</script>
...
<template>
  <section id="projects" class="py-24 font-sans">
    <div class="container mx-auto px-6 max-w-300">
      <h2 class="text-renome text-[36px] font-bold uppercase mb-16 tracking-tight">
      ...
      <div class="flex flex-col gap-10">
        ...
        >
          ...
          <div class="flex-1 w-full">
            <div class="rounded-xl overflow-hidden shadow-[0_5px_20px_rgba(0,0,0,0.1)] border border-gray-100 bg-white">
              <img 
                v-if="getImg(project)"
                :src="getImg(project)" 
              ...
              />
              <div v-else class="aspect-video bg-gray-50 flex items-center justify-center text-gray-300">
                Скриншот интерфейса
              </div>
            </div>
          </div>
