...
<template>
  ...
  <p v-if="item.type == 'paragraph'">
    <RtfItem v-for="child in item.children" :item="child" :useAccentColor="useAccentColor"></RtfItem>
  </p>
  <h3
  ...
  >
    <span :class="useAccentColor?'bg-renome-gradient':'bg-current'" class="w-2 h-2 mt-1.5 shrink-0"></span>
    <RtfItem v-for="child in item.children" :item="child" :useAccentColor="useAccentColor"></RtfItem>
  </li>
</template>
