...
<template>
  <section id="services" class="py-20 bg-transparent relative z-10 font-sans">
    <div class="container mx-auto px-6 max-w-[1200px]">
      ...
      <h2 class="text-[#0A4436] text-[36px] font-medium uppercase mb-10">УСЛУГИ</h2>

      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        
        <!-- 1. ГЛАВНАЯ УСЛУГА -->
        <div v-if="mainService" class="md:col-span-3 bg-white p-10 rounded-[32px] shadow-2xl flex flex-col md:flex-row gap-10 border border-gray-100">
          <div class="flex-1">
            <div class="flex items-start gap-4 mb-6">
              <div class="w-75 h-75  rounded-xl flex items-center justify-center shrink-0">
                <img v-if="mainService.icon" :src="`http://79.174.80.177:1337${mainService.icon.url}`" class="w-20" />
              </div>
              <h3 class="text-[#0A4436] text-[28px] font-bold uppercase leading-tight">
                {{ mainService.name }}
              </h3>
            </div>
            ...
            <button class="flex items-center gap-4 bg-renome-gradient text-white px-6 py-3 rounded-full hover:opacity-90 transition-all">
          ...
          <div class="hidden md:block w-[350px] shrink-0">
             <img v-if="mainService.picture" :src="`http://79.174.80.177:1337${mainService.picture.url}`" class="w-full object-contain" />
          </div>
        </div>

        <!-- 2. ОСТАЛЬНЫЕ УСЛУГИ (3 в ряд) -->
        <div v-for="service in otherServices" :key="service.id" 
          ...
          <div class="flex items-center gap-4 mb-6">
             <div class="w-12 h-12 rounded-lg flex items-center justify-center shrink-0">
                <img v-if="service.icon" :src="`http://79.174.80.177:1337${service.icon.url}`" class="w-20" />
             </div>
             <h3 class="text-[#0A4436] text-[18px] font-bold uppercase leading-tight">
               {{ service.name }}
             </h3>
          </div>
          ...
          <button class="flex items-center justify-between bg-renome-gradient text-white px-5 py-3 rounded-full w-fit gap-4 hover:opacity-90 transition-all mt-auto">
